...
/*******************************************************************************
* Function Name: NOTIFY_CLOCK_Start
********************************************************************************
*
* Summary:
...
void NOTIFY_CLOCK_Start(void) 
{
    ...
    NOTIFY_CLOCK_CLKEN |= NOTIFY_CLOCK_CLKEN_MASK;
	NOTIFY_CLOCK_CLKSTBY |= NOTIFY_CLOCK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: NOTIFY_CLOCK_Stop
********************************************************************************
*
* Summary:
...
void NOTIFY_CLOCK_Stop(void) 
{
    ...
    NOTIFY_CLOCK_CLKEN &= (uint8)(~NOTIFY_CLOCK_CLKEN_MASK);
	NOTIFY_CLOCK_CLKSTBY &= (uint8)(~NOTIFY_CLOCK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: NOTIFY_CLOCK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((NOTIFY_CLOCK_CLKEN & NOTIFY_CLOCK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = NOTIFY_CLOCK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = NOTIFY_CLOCK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* NOTIFY_CLOCK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(NOTIFY_CLOCK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(NOTIFY_CLOCK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: NOTIFY_CLOCK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: NOTIFY_CLOCK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = NOTIFY_CLOCK_GetSourceRegister();
    uint16 oldDivider = NOTIFY_CLOCK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = NOTIFY_CLOCK_CLKEN & NOTIFY_CLOCK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(NOTIFY_CLOCK_DIV_PTR, clkDivider);
                NOTIFY_CLOCK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                NOTIFY_CLOCK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(NOTIFY_CLOCK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((NOTIFY_CLOCK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: NOTIFY_CLOCK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(NOTIFY_CLOCK_DIV_PTR);
}


/*******************************************************************************
* Function Name: NOTIFY_CLOCK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    NOTIFY_CLOCK_MOD_SRC |= modeBitMask & (uint8)NOTIFY_CLOCK_MODE_MASK;
}


/*******************************************************************************
* Function Name: NOTIFY_CLOCK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    NOTIFY_CLOCK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(NOTIFY_CLOCK_MODE_MASK));
}


/*******************************************************************************
* Function Name: NOTIFY_CLOCK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return NOTIFY_CLOCK_MOD_SRC & (uint8)(NOTIFY_CLOCK_MODE_MASK);
}


/*******************************************************************************
* Function Name: NOTIFY_CLOCK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = NOTIFY_CLOCK_GetDividerRegister();
    uint8 oldSrc = NOTIFY_CLOCK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        NOTIFY_CLOCK_MOD_SRC |= CYCLK_SSS;
        NOTIFY_CLOCK_MOD_SRC =
            (NOTIFY_CLOCK_MOD_SRC & (uint8)(~NOTIFY_CLOCK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: NOTIFY_CLOCK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return NOTIFY_CLOCK_MOD_SRC & NOTIFY_CLOCK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: NOTIFY_CLOCK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    NOTIFY_CLOCK_PHASE = clkPhase & NOTIFY_CLOCK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: NOTIFY_CLOCK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return NOTIFY_CLOCK_PHASE & NOTIFY_CLOCK_PHASE_MASK;
}
