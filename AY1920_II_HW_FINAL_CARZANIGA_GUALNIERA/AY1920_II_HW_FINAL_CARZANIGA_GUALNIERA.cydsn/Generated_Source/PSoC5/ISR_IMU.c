...
void ISR_IMU_Stop(void)
{
    ...
    ISR_IMU_SetVector(&IntDefaultHandler);
}
...
void ISR_IMU_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)ISR_IMU__INTC_NUMBER] = address;
}
...
cyisraddress ISR_IMU_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)ISR_IMU__INTC_NUMBER];
}
...
uint8 ISR_IMU_GetState(void)
{
    ...
    return ((*ISR_IMU_INTC_SET_EN & (uint32)ISR_IMU__INTC_MASK) != 0u) ? 1u:0u;
}
