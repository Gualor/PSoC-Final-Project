...
{
   #if(MAIN_TIMER_UsingFixedFunction)
       return ((uint32)CY_GET_REG16(MAIN_TIMER_PERIOD_LSB_PTR));
   #else
       return (CY_GET_REG32(MAIN_TIMER_PERIOD_LSB_PTR));
   #endif /* (MAIN_TIMER_UsingFixedFunction) */
}
...
void MAIN_TIMER_WritePeriod(uint32 period) 
{
    ...
    #else
        CY_SET_REG32(MAIN_TIMER_PERIOD_LSB_PTR, period);
...
{
   #if(MAIN_TIMER_UsingFixedFunction)
       return ((uint32)CY_GET_REG16(MAIN_TIMER_CAPTURE_LSB_PTR));
   #else
       return (CY_GET_REG32(MAIN_TIMER_CAPTURE_LSB_PTR));
   #endif /* (MAIN_TIMER_UsingFixedFunction) */
}
...
void MAIN_TIMER_WriteCounter(uint32 counter) 
{
    ...
    #else
        CY_SET_REG32(MAIN_TIMER_COUNTER_LSB_PTR, counter);
